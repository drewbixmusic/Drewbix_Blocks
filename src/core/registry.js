...
export const MOD = {
  ...
  mv_regression: {
    ...
    cfg: {
      ...
      key_modifier:     { t: 'text',  d: '_',      l: 'Key Modifier Separator (strips suffix e.g. _ → SPY_1 becomes SPY; also used for set detection)' },
      seed:             { t: 'number', d: 42, l: 'Random Seed' },
      validation_mode:  { t: 'sel',   opts: ['Standard', 'Segment Ensemble'], d: 'Standard', l: '── Validation Mode ──' },
      fold_source:      { t: 'sel',   opts: ['By Sets', 'Stratify All'], d: 'By Sets', l: '[Ensemble] Fold Source — "By Sets" auto-detects set modifiers (e.g. _1…_10) and falls back to x-bands. "Stratify All" ignores modifiers and splits the full dataset by x-value bands (frankenstein folds — mixes all time periods, helps generalize when sets are limited).' },
      sets_per_fold:    { t: 'sel',   opts: ['1', '2', '3', '4', '5'], d: '2', l: '[Ensemble] Sets per Fold — how many modifier sets each segment model trains on. Balanced cyclic assignment ensures every set appears equally across folds. Default 2 mixes consecutive + spread time windows to reduce time-latching.' },
      fallback_x_field: { t: 'dynfield', d: 't_rel', l: '[Ensemble] Fallback / Stratify X Field' },
      fallback_bands:   { t: 'sel',   opts: ['5', '8', '10', '15', '20'], d: '10', l: '[Ensemble] Fallback / Stratify Band Count' },
      blend_mode:       { t: 'sel',   opts: ['NNLS', 'Standard MV', 'RSQ Blend', 'Best'], d: 'NNLS', l: '── Segment Aggregator — how segment predictions are combined. NNLS: non-negative weighted blend (robust). Standard MV: unconstrained OLS blend (allows negative weights). RSQ Blend: simple OOS R²-weighted average (fast fallback). Best: use only the highest-OOS-R² segment (single model).' },
    },
  },
  rand_forest: {
    ...
    cfg: {
      ...
      max_trees:              { t: 'sel',   opts: ['5', '10', '15', '20', '25', '50', '100', '200', '400', '800'], d: '25', l: 'Max Trees per Fold (total = k × this)' },
      convergence_threshold:  { t: 'sel',   opts: ['2%', '5%', '10%', '20%', '33%', 'Off'], d: '10%', l: 'Convergence Exit Threshold (Off = run to max)' },
      max_depth:              { t: 'sel',   opts: ['3', '4', '5', '6', '8', '10', 'unlimited'], d: '5', l: 'Max Depth' },
      min_samples_leaf_pct:   { t: 'sel',   opts: ['0.09%', '0.18%', '0.35%', '0.70%', '1.40%'], d: '0.35%', l: 'Min Samples/Leaf (% of training rows)' },
      min_samples_split_ratio:{ t: 'sel',   opts: ['1.5:1', '2:1', '2.5:1', '3:1'], d: '2:1', l: 'Min Samples to Split (ratio × leaf)' },
      split_candidates:       { t: 'sel',   opts: ['20', '50', '100', '200', 'All'], d: '100', l: 'Split Candidates (speed vs quality)' },
      feat_eng:               { t: 'bool',  d: false, l: 'Internal Interactions (mul/div of top N features)' },
      eng_top:                { t: 'sel',   opts: ['3', '5', '8', '10'], d: '5', l: 'Interaction Top N Features' },
      key_field:              { t: 'text',  d: 'symbol', l: 'Key Field (for per-key diagnostics)' },
      key_modifier:           { t: 'text',  d: '_', l: 'Key Modifier Separator (strips suffix e.g. _ → SPY_1 becomes SPY; also used for set detection)' },
      seed:                   { t: 'number',d: 42, l: 'Random Seed' },
      validation_mode:        { t: 'sel',   opts: ['Standard', 'K-fold Enhanced'], d: 'Standard', l: '── Validation Mode ──' },
      test_pct:               { t: 'sel',   opts: ['10%', '15%', '20%', '25%', '30%'], d: '20%', l: '[Standard] Test Split %' },
      fold_source:            { t: 'sel',   opts: ['By Sets', 'Stratify All'], d: 'By Sets', l: '[K-fold] Fold Source — "By Sets" auto-detects set modifiers (e.g. _1…_10) and falls back to x-bands. "Stratify All" ignores modifiers and splits the full dataset by x-value bands (frankenstein folds — mixes all time periods, helps generalize when sets are limited).' },
  ...
  pred_normalize: {
    ...
    cfg: {
      key_field:        { t: 'dynfield', d: 'symbol',  l: 'Key Field' },
      model_t_field:    { t: 'dynfield', d: 't_rel',   l: 'Model Time Field (t0 reference, from model input)' },
      ...
      gain_factor:      { t: 'sel', opts: ['0.10', '0.25', '0.33', '0.50', '0.67', '0.75', '1.00', '1.50', '2.00'], d: '0.33', l: 'Vol Gain Factor (multiplier on model volatility target — 0.33 = ⅓ of model vol)' },
